...
// --- GLOBALS ---
LockFreeMPSC<Envelope, QUEUE_SIZE> global_queue;
...
void network_thread(int id, int core_id) {
    ...
    while (running) {
        if (receiver.receive(raw)) {
            ...
            // Lock-Free Push (With Backpressure tracking)
            while (!global_queue.push(env)) {
...
void strategy_thread(int core_id) {
    pin_to_core(core_id);
    AsyncLogger logger;
    std::cout << "[Strategy] MPSC Engine Started. Logging metrics..." << std::endl;
